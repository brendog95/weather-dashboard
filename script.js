...
var cityName,
    ...
    fiveDayQueryParams,
    fiveDayList;
//cupdate a variable with query parameters integrated for current weather
...
function renderCities() {
    ...
    for (var i = 0; i < cities.length; i++) {
        ...
        button.attr("style", "width: 25%")
        button.addClass("btn btn btn-outline-dark text-center");
        li.append(button);
        $("#city-list").prepend(li);
        $("#city-list").prepend("<br>");
    }
}
//on site load apply previously searched cities to the ul for cities
function onInit() {
    $("#city-list").empty();
...
//submits the city selected to be put into the API URL and calls the Ajax functions
$("#search-btn").on("click", function (event) {
    ...
    $.ajax({
        ...
        currentWeather();
        //update a variable with query parameters integrated for 5 day forecast
        ...
        $.ajax({
            ...
            //trying to get the uv index to display a message depending on the rating
            /*if (uvIndexEl <= 2) {
                $(".uv-warning").append("Minimal - Wearing a hat is sufficient protection.")
            }
            if (uvIndexEl > 2 <=4 ) {
                $(".uv-warning").append("Low - Wearing a hat and a sunscreen with SPF 15 is recommended.")
            }
            if (uvIndexEl > 4 <= 6 ) {
                $(".uv-warning").append("Moderate - Wearing a hat, a sunscreen with SPF 15 and staying in the shade is recommended.")
            }
            if (uvIndexEl > 6 <= 9) {
                $(".uv-warning").append("High - In addition to the precautions recommended above, it is advised to stay indoors between 10 a.m. and 4 p.m.") 
            }
            if (uvIndexEl > 9 ) {
                $(".uv-warning").append("Very High - In addition to the precautions recommended above, it is advised to stay indoors if possible.") 
            }*/
        })
